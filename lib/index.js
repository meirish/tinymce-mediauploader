(function (document) {
  ...
  function getMatchers(editor) {
    ...
    function createMedia(tagName, src) {
      ...
      var img_onload = getParam(editor, 'img_onload');
      if (tagName === 'img' && typeof img_onload === 'function') {
        media.onload = img_onload;
      }
      media.src = src;
      return media;
    }
  ...
  function uploadFile(editor, file) {
    ...
    if (typeof upload_file === 'function') {
      upload_file(getDOMNode(editor), file, function handleUrl(fileUrl, error) {
        if (!error) {
  ...
  function embedMedia(editor, mediaUrl) {
    ...
    var element = getDOMNode(editor);
    var matchers = getMatchers(editor);
    var media = null;
